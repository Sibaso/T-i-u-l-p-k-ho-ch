...
//								+ "," + t + "]");
		...
		for(int t=1;t<=T;t++) {
			for(int i=0;i<N;i++) {
				...
				for(int j=0;j<N;j++) {
					e.setCoefficient(X[i][j][t], 1);
					e.setCoefficient(X[j][i][t], 1);
				}
			}
		}
		
		for(int i=0;i<N;i++) {
			MPConstraint g = solver.makeConstraint(0,0);
			g.setCoefficient(D[i], -1);
			for(int j=0;j<N;j++) {
				if(i!=j) {
					MPConstraint e = solver.makeConstraint(1,1);
					MPConstraint f = solver.makeConstraint(1,1);
					for(int t=1;t<=T;t++) {
						e.setCoefficient(X[i][j][t], 1);
						f.setCoefficient(X[j][i][t], 1);
					}
				}
				for(int k=0;k<N;k++) {
					for(int t=1;t<=T;t++) {
						g.setCoefficient(F[i][j][k][t], d[j][k]);
					}
				}
			}
		}
...
//						}
			...
			System.out.println("D: "+D[i].solutionValue());
			}
			System.out.println("-------------");
		}
